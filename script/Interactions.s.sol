 ...
 * @notice Creates a VRF subscription for the KuriCore contract
 */
...
/**
 * @title AddConsumer
 * @notice Adds the KuriCore contract as a consumer to a VRF subscription
 */
contract AddConsumer is Script {
    ...
    function addConsumerUsingConfig(address kuriCoreAddress) public {
        HelperConfig helperConfig = new HelperConfig();
        uint256 subId = helperConfig.getConfig().subscriptionId;
        address vrfCoordinatorV2_5 = helperConfig.getConfig().vrfCoordinatorV2_5;
        address account = helperConfig.getConfig().account;

        addConsumer(kuriCoreAddress, vrfCoordinatorV2_5, subId, account);
    }

    function run() external {
        address mostRecentlyDeployed = DevOpsTools.get_most_recent_deployment("KuriCore", block.chainid);
